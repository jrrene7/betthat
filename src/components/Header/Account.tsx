    ...
    <div className="flex items-center gap-3">
      {data?.user && meUser && (
        <div className="hidden items-center gap-1 rounded-full border border-[#2f2f2f] bg-[#1a1a1a] px-3 py-1 md:flex">
          ...
          <span className="text-sm font-semibold text-yellow-400">{meUser.balance.toLocaleString()}</span>
        ...
        <Tippy interactive placement="bottom-end" render={() => <Dropdown userId={data?.user?.id} />}>
          <div className="cursor-pointer">
            <div className="h-8 w-8">
              <Avatar src={meUser?.image ?? data?.user?.image} className="h-8 w-8 rounded-full" />
            </div>
          </div>
        </Tippy>
      )}
